export default function () {
  ...
  if(localStorage.getItem('weatherResponse') &&  (Date.now() - localStorage.getItem('responseTimestamp')) < tenMinutes) {
    ...
    console.log('Last logged at ' + new Date(parseInt(localStorage.getItem('responseTimestamp'), 10)));
  }
  else {
    ...
    localStorage.setItem('responseTimestamp', Date.now());
    console.log('Logged at ' + new Date(parseInt(localStorage.getItem('responseTimestamp'), 10)).toString());
